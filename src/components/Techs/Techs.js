function Techs() {
  return (
    <section className="techs">
      <p className="title techs__title">Технологии</p>
      <div className="techs__content">
        <p className="techs__second-title">7 технологий</p>
        <p className="techs__description">На курсе веб-разработки мы освоили технологии, которые
          применили в дипломном проекте.</p>
      </div>
      <div className="tech__list">
        <div className="tech__item">
          <p className="tech__item-title">HTML</p>
        </div>
        <div className="tech__item">
          <p className="tech__item-title">CSS</p>
        </div>
        <div className="tech__item">
          <p className="tech__item-title">JS</p>
        </div>
        <div className="tech__item">
          <p className="tech__item-title">React</p>
        </div>
        <div className="tech__item">
          <p className="tech__item-title">Git</p>
        </div>
        <div className="tech__item">
          <p className="tech__item-title">Express.js</p>
        </div>
        <div className="tech__item">
          <p className="tech__item-title">mongoDB</p>
        </div>
      </div>
    </section>
  );
}

export default Techs;
